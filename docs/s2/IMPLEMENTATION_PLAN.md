...
5. 新旧账本、目标摘要及单 writer 状态一致后，按 manifest 恢复已准入任务生产者。
6. 保存完整前后证据，逐项评定 S2-R01–R10；后续继续使用范围不得扩大到未准入 A2 作业。
...
| S2-V04 | R06 → A05 → P03/P06 | 未执行与已执行两类回退、未决状态不重放 |
| S2-V05 | R07/R08 → A06 → P05 | 端到端结果、连续日志、严格 outbox、重启与身份 |
