...
| S2-R03 | 同一目标只有一个执行者 | 新实例启动前旧 Worker、子进程和重启来源均已停；v1 生产者与新 MCP/CLI job 准入分别冻结 |
...
| S2-R06 | 部署失败可以核对地回退 | 先停全部新执行入口、排队启动和进程树；分别保存 v1/job 事实，未知屏障不因回退解除 |
...
| 新 job 后端 | authority/账本实例/锁锚点、epoch、MCP/CLI 准入开关、排队启动、cgroup、job 事件/租约/证据、容量与撤销策略 |
...
当 E5 采用包含新 broker 的候选时，旧 v1 与新 job 的健康、冻结和恢复均须分别证明；
只取得三项 v1 健康结果不能宣称整个 E5 已验收。S2 本身不隐含新增 job 的操作权限。
