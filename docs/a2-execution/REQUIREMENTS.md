...
| AX-R02 | A2 操作有固定契约 | 固定类型、跨语言一致的输入与摘要、显式环境和影响根；逐作业及全局容量受限；未知字段拒绝 |
| AX-R03 | 重连不产生重复副作用 | 同 id、同 digest 返回原作业；同 id 异内容冲突；提交超时用原 id 查询；不保证外部系统 exactly-once |
| AX-R04 | 崩溃、取消和结果不明可核对 | 持久意图、启动/取消围栏、进程归属与副作用分开；当前无进程不代表以后不会启动；核对不自动续跑 |
| AX-R05 | 只有一个资源所有者 | 固定后端根、资源租约、受保护 epoch 与进程退出证明；不同服务／state root 不构成隐含互斥 |
| AX-R06 | NAS 恢复验证真实且删除受限 | 后端强制同候选前置证据准入，再核对真实挂载、发布与独立回读；持续独占合成范围，异常即停并保留可能部分删除的事实 |
| AX-R07 | 完整证据私有、可取回、可校验 | 私有封存 manifest、ZIP、摘要、成员清单；分块续传重建文件；报告和副作用状态分开 |
| AX-R08 | 鉴权与撤销覆盖所有工具 | token 验签与每次用户/scope/归属授权均通过；本地撤销即时生效，issuer 撤销传播时限如实验证；资源 ID 或内容指令不授予权限 |
...
失败保留覆盖 broker 的账本、日志、作业根和实际残留；固定上游测试自行清理的临时夹具明确列为
EPHEMERAL_BY_UPSTREAM_TOOL，不能虚构完整现场。现有工具无法保证的保留要求须先另行补齐工具基线。
