...
| ID | 要求 | 判定依据 |
| --- | --- | --- |
| AX-R01 | 来源和权限在执行前匹配 | 节点、安装、epoch、策略、目录与固定输入不符即拒绝；接口返回主体获准的逻辑引用和准确绑定，客户端对照私有准入目标，不自动改绑；执行后的来源核验不能替代 |
...
| AX-R04 | 崩溃、取消和结果不明可核对 | 持久意图、启动/取消/撤销围栏、进程归属与副作用分开；当前无进程不代表以后不会启动；阻塞存储检查不占控制面或围栏；核对不自动续跑 |
