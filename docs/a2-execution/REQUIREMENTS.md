...
| ID | 要求 | 判定依据 |
| --- | --- | --- |
| AX-R01 | 来源和权限在执行前匹配 | 节点、安装、epoch、策略、目录与固定输入不符即拒绝；受限预检与业务执行分别登记意图，预检合格后才准入业务启动；接口返回主体获准的逻辑引用和准确绑定，客户端对照私有准入目标，不自动改绑 |
| AX-R02 | A2 操作有固定契约 | 固定类型、跨语言一致的输入与摘要、显式环境和影响根；逐作业及全局容量受限；未知字段拒绝 |
| AX-R03 | 重连不产生重复副作用 | 同 id、同 digest 返回原作业；同 id 异内容冲突；业务与核对各有稳定请求身份，超时按原 id 查询，明确区分重取与新观察；不保证外部系统 exactly-once |
| AX-R04 | 崩溃、取消和结果不明可核对 | 持久意图、启动/取消/撤销围栏、进程归属与副作用分开；取消明确指向业务或某轮核对；未查到记录不证明未来不会受理；阻塞存储检查不占控制面或围栏；核对不自动续跑 |
...
| AX-R08 | 鉴权与撤销覆盖所有工具 | token 验签与每次用户/scope/归属授权均通过；本地撤销立即禁止新准入和启动授权，另报排队启动封阻与运行中停止结果；issuer 撤销传播时限如实验证；资源 ID 或内容指令不授予权限 |
| AX-R09 | 三种接入实际可用且可独立升级 | Connector 保留；MCP 从引用发现、prepare 结果衔接到证据交付完整验证，无须人工转抄；Plugin 绑定契约版本；未具备客户端支持明确标记阻塞 |
