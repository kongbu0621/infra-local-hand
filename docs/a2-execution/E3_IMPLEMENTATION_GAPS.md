...
2026-09-25 接续索引：下文是上述准确旧源码的历史核查，不能用其中“未实现”判断最新源码。
管理侧 observer、普通客户端及 Q2 三阶段实现见 [Q2 整链](E3_QUOTA_Q2_CHAIN.md)，
当前入口、批量前检与保留证据复核见 [接续说明](E3_QUOTA_Q2_HANDOFF.md)。
源码实现与实际验收分别记录；真实 H01–H13、最终容量/退出及生产支持仍未完成。
