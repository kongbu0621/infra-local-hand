...
| --- | --- | --- | --- |
| AX-V01 | R01/R10 → A02/A03/A09 | 伪 source、替换 wheel/外部脚本、脏树、prepared 漂移、错误安装/epoch/profile/registry 均在执行前拒绝 | E5 从 Git blobs 到 wheel、插件、实际进程的完整来源；不是只比 core |
| AX-V02 | R02/R08 → A01/A02/A03 | 未知 kind/字段、任意路径/argv/env/URL、遍历、链接、重复 key、非法 Unicode、超量请求拒绝；CLI 同权限 | E5 allowlist、账户、网络/凭据隔离及预算实际生效 |
| AX-V03 | R03 → A03/A04 | 同 id 同 digest 去重、异内容冲突、跨主体拒绝、接受响应丢失、过期与重连；意图后崩溃不重放 | E4 客户端断线与 token 更新仍沿用原作业；E5 重启保留账本 |
| AX-V04 | R04 → A04/A05 | intent/spawn、子进程退出/状态保存交界；PID 复用、broker 死亡、cancel/完成竞争、UNKNOWN 核对不续跑 | E5 实际 cgroup/监督器与恢复证据，停止不声明 NAS 回滚 |
...
| AX-V07 | R07 → A04/A07 | writer 存活不能完整 seal；冻结事件序号/成员、半写不可见、并发同名不覆盖、fsync/DB 不明、seal 无自引用、ZIP 安全、成员替换拒绝 | E4 真实 ZIP、seal、manifest、逐件哈希与 ZIP 完整性核对 |
| AX-V08 | R07/R09 → A07/A08 | 分页、chunk 上限、错 offset/digest、乱序/重复/断线续取、日志截断如实记录、磁盘满停止 | E4 ≥16 MiB 非高压缩率 ZIP 实际取回，不能只有文本哈希 |
| AX-V09 | R08/R09 → A03/A07/A08 | 首次未链接→发现/挑战→PKCE→授权→调用、撤销/再授权；错 issuer/aud/scope/过期、跨作业越权、恶意日志指令拒绝 | E4 实际工作区、用户授权、Tunnel/HTTPS 和逐块撤销；缺权限不能跳过 |
| AX-V10 | R09/R10 → A08/A09 | 插件版本/contract 不符阻止提交、无私有ID/secret/hooks、核心无 MCP 依赖也可安装 | E4 私有连接映射和当前客户端支持；公网目录上架不在验收内 |
| AX-V11 | R11 → A04/A09 | 旧 v1 行为保持、历史 Task 不因新 job 重放、回退保留新账本、旧版读不懂则阻塞 | E5 S2 全账本迁移与实际回退验收，不覆盖旧环境 |
...
编译只针对获准新增后的实际 Python 源码，文档提交不声称已编译新实现。
...
| OAuth issuer、client 注册方式、redirect URI、audience、授权主体/scope、撤销策略 | 尚未冻结 | E4；测试 issuer 不可用于真实准入 |
...
| 每类作业 wall-time/CPU/RSS/pids/磁盘/NAS/日志预算，证据保存和容量 | 未在真实主机定值 | E5；E1–E3 采用有限 fixture 预算但不外推性能 |
