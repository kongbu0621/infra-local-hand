...
3. 将架构 A02 的准确固定调用模板编入六类 job registry，冻结参数、显式环境、可写根、阶段及预算规则，建立严格 schema。
   ...
   区分受限预检意图与业务执行意图：预检先受监督执行，事实合格后才登记业务意图；绑定失效即阻塞。
...
5. 数据库意图／spawn、退出／状态、证据 rename/fsync／seal 的每个交界模拟崩溃、I/O 失败和响应丢失。
   ...
   按客户端 reconcile_id 持久防重，查询与取消明确寻址同一轮；丢回执重取不变成新观察。
...
| --- | --- | --- | --- |
| AX-V01 | R01/R06/R10 → A02/A03/A05/A07/A09 | 伪来源/替换工具/prepared 漂移/错部署拒绝；预检许可不授予业务启动，事实失效或预检未决不得写业务意图；发现值不符私有目标/authority 或 STALE 不自动改绑重投；NAS 缺前置证据/异候选环境/更新失败或未决均拒绝 | E4 连接与目标绑定；E5 全链来源；E6 真实先决证据绑定 |
...
| AX-V03 | R03 → A03/A04/A07 | 同 id 同 digest 去重、异内容冲突、跨主体拒绝、接受响应丢失、过期与重连；核对受理回执丢失/完成后重送复用原轮，显式新 id 才新观察；NOT_FOUND 不换 id，I/O 不明不当缺失；意图后崩溃不重放 | E4 客户端断线与 token 更新仍沿用原业务/核对 id；E5 重启保留账本 |
| AX-V04 | R04 → A04/A05/A07 | 排队取消、intent/spawn 间取消、延迟启动回执及重启；退出/状态交界、PID 复用、broker 死亡、cancel/完成竞争；取消目标隔离，旧 job 取消重试不影响新核对，目标缺失不报取消完成；挂起 helper 时 status/cancel 可受理，UNKNOWN 不续跑或释放资源 | E5 已排队启动撤销与实际 cgroup/监督器证据；E6 隔离真实存储故障下控制面可用性 |
...
| AX-V08 | R02/R07/R09 → A05/A07/A08 | 分页/chunk/错偏移与摘要/断线；宿主回调→writer 不取 token；多作业及重复 reconcile 配额，同父核对互斥/旧轮未决不新建 helper，日志截断、磁盘满停止 | E4 当前 Work 原始结果接桥、≥16 MiB ZIP 实际取回，无能力则 BLOCKED |
